...
class SochDBError(Exception):
    """
    Base exception for SochDB errors.
    
    All SochDB exceptions inherit from this class, providing:
...
class ConnectionError(SochDBError):
    """Failed to connect to SochDB server or open database."""
    code = ErrorCode.CONNECTION_FAILED


class TransactionError(SochDBError):
...
class ProtocolError(SochDBError):
    """Wire protocol error."""
    code = ErrorCode.PROTOCOL_ERROR


class DatabaseError(SochDBError):
...
class NamespaceError(SochDBError):
...
class CollectionError(SochDBError):
...
class ValidationError(SochDBError):
...
class EmbeddingError(SochDBError):
    ...
    def __init__(self, message: str = "Embedding feature not available"):
        super().__init__(
            message,
            remediation="Install embeddings extra: pip install sochdb-client[embeddings]",
        )
...
def from_rust_error(code: int, message: str, context: Optional[Dict[str, Any]] = None) -> SochDBError:
    """
    ...
    """
    error_class = _ERROR_MAP.get(code, SochDBError)
